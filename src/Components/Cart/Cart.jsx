...
export const Cart = () => {
  ...
  let Cartdata = JSON.parse(localStorage.getItem("flowerCart"));
  console.log("This is reattt", Cartdata);
  ...
  const priceChange = (dataItems) => {
    const total =
      dataItems.length >= 1
        ? dataItems.reduce(
            (value, item) => item.productID.mrp * item.quantity + value,
            0
          )
        : "";
    setTotalPrice(total);
  ...
  const quantityChange = (dataItems) => {
    const qty =
      dataItems.length >= 1
        ? dataItems.reduce((value, item) => item.quantity + value, 0)
        : "";
    setQuantity(qty);
      ...
      >
        ...
        >
          ...
          <Stack spacing="6">
            ...
            {/* {Cartdata !== "No CartItems Found from this userID"
              ? Cartdata?.map((item, index) => (
                  <CartItem
                    key={item.id}
                    {...item}
                    onChangeQuantity={onChangeQuantity.bind(this, item)}
                    onClickDelete={onClickDelete.bind(this, index)}
                  />
                ))
              : ""} */}
          </Stack>
